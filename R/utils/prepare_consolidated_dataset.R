...
build_variable_dictionary <- function() {
  data.frame(
    ...
    mapping_note = c(
      ...
      "Used directly; reconstructed into N1/N2 faculty context so rows are not reshaped again.",
      "Used directly; reconstructed into N1/N2 faculty context so rows are not reshaped again.",
      "Used directly; equals faculty_player in victim rows and remains explicit in bystander rows.",
      "Legacy role-relative grouping from the source file; retained only for auditing, not as the main H2/H3/H5 predictors.",
      "Legacy role-relative grouping from the source file; retained only for auditing, not as the main H2/H3/H5 predictors.",
