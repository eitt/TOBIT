...
build_variable_dictionary <- function() {
  data.frame(
    ...
    mapping_note = c(
      ...
      "Source/legacy target-decision field from the input file; mapped to active analytical alias accept_target (1 = accept, 0 = reject) for the row-dynamic target actor.",
      "Source/legacy counterpart-decision field from the input file; mapped to active analytical alias accept_other (1 = accept, 0 = reject) for the non-target actor in the same row.",
