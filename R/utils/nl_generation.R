...
# Purpose: Generate dynamic narrative text explaining Tobit model coefficients
# Dependencies: model_functions.R

# Helper to explain p-value significance
describe_significance <- function(p_value, estimate) {
  if (is.na(p_value)) return("the p-value cannot be determined")
...
get_term_definition <- function(term) {
  ...
  if (term %in% names(meaning_map)) return(meaning_map[[term]])
  if (grepl("^factor\\(negotiator_slot\\)", term)) return("fixed effects for specific negotiator presentation order")
  if (grepl("^factor\\(stage\\)", term)) return("fixed effects for specific chronological evaluation stages")
  return(sprintf("the term '%s'", term))
}

#' Generate fully formed narrative interpreting the tabular coefficients
generate_coefficient_narrative <- function(coef_df) {
  lines <- c()
  
  for (i in 1:nrow(coef_df)) {
    term <- coef_df$term[i]
    term_label <- coef_df$label[i]
    est <- coef_df$estimate[i]
    p_val <- coef_df$p_value[i]
    
    # Handle the standard standard intercepts and scales that Tobit models produce
    if (term == "(Intercept)") {
