...
- `R/hypotheses/`: Hypothesis estimation scripts that fit both the primary Tobit model and the non-parametric robustness companion for each specification (`H1_test.R`, `H2_test.R`, etc.).
...
6. **Hypothesis tests** - H1 through H3 standalone model runs, each generating Tobit outputs plus the cluster-aware non-parametric robustness fit, using judgment-level relational structure terms rather than descriptive case labels
